...
Lemma stage_1_correct : forall (a b : Z),
  (a >= 0)%Z /\ (b >= 0)%Z ->
  wp s1 (wp_normal (Q1 a b)) (upd (upd empty_state "a"%string (VZ a)) "b"%string (VZ b)).
Proof.
  intros a b [Ha Hb]. wp_reduce. split.
  - unfold lget, upd, updZ; cbn. split; [apply Ha | reflexivity].
  - intro r. intro Hr. split.
    + unfold Q1, wp_normal; cbn. rewrite Hr. repeat split; auto.
...
Proof.
  ...
  - intro r. intro Hr. split.
    + unfold Q2, wp_normal; simpl. unfold lget, asZ in *.
...
Theorem frame_two_calls_staged : forall (a b : Z),
  ...
  wp (CSeq (CSeq s1 s2) (CAss "result"%string (APlus (AVar "a2"%string) (AVar "b2"%string))))
     (wp_normal (fun s => (asZ (s "result"%string) = ((asZ (s "a"%string) + asZ (s "b"%string)) + 2)%Z)))
     ((upd (upd empty_state "a"%string (VZ a)) "b"%string (VZ b))).
Proof.
  intros a b [Ha Hb].
  apply (wp_seq_decompose_normal s1 (CSeq s2 (CAss "result"%string (APlus (AVar "a2"%string) (AVar "b2"%string)))) (Q1 a b) (wp_normal (fun s => (asZ (s "result"%string) = ((asZ (s "a"%string) + asZ (s "b"%string)) + 2)%Z))) _).
  { apply stage_1_correct. split; assumption. }
  { intros s1 Hq. unfold Q1 in Hq.
    destruct Hq as [Ha2_eq [Ha2_v [Ha_eq [Ha_v [Hb_eq Hb_v]]]]].
    apply (wp_seq_decompose_normal s2 (CAss "result"%string (APlus (AVar "a2"%string) (AVar "b2"%string))) (Q2 a b) (wp_normal (fun s => (asZ (s "result"%string) = ((asZ (s "a"%string) + asZ (s "b"%string)) + 2)%Z))) _).
