...
(** Apply a per-loop lemma generated by the iris_proof_gen pipeline.
    Parameterised by [lemma_name], [l] (the heap location), and [n]
    (the loop bound).  Wraps [reshape_expr], [wp_bind], [iApply]
    with the lemma, and the exit-wand subgoal. *)
Ltac apply_while_lemma lemma_name l n :=
  snakelet_popvals;
  lazymatch goal with
  | |- envs_entails _ (wp ?s ?E ?e ?Q) =>
    lazymatch e with
    ...
    iApply (lemma_name s E l n 0%Z
      (λ v, WP (Val v ;; Load (Val (LitLoc l))) @ s; E
        {{ v', ⌜(v' = LitInt n)⌝ }}%I)%I
      with "[] [] []");
    [iFrame; iPureIntro; lia
    | iIntros "Hc_n";
      snakelet_pure_step; iNext; snakelet_simpl;
      iApply (wp_load s E l (LitInt n)
        (λ v', ⌜(v' = LitInt n)⌝ )%I
        with "Hc_n");
      [ iIntros "_"; iPureIntro; reflexivity ]
    ]
  | _ => fail "apply_while_lemma: goal is not a WP"
  end.
